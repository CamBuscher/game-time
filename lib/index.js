...
function generateBricks() {
  if (newGame.level === 1 || newGame.level === 2) {
    let newBricks = newGame.createBricksLvlsOneTwo(40, newGame.level);
    newBricks.forEach( brick => newGame.grabBricks(brick) );
  } else if (newGame.level === 3) {
    let newBricks = newGame.createBricksLvlThree(54);
    newBricks.forEach( brick => newGame.grabBricks(brick) );
  }
}
...
function ballDeath() {
  if(requestID) {
    ...
    requestID = null;
    isDead = false;
    let livesIndicator = document.querySelector('.lives-indicator');
